...
class GraphLayer(nn.Module):
    ...
    def forward(self, Z, D, new_mask, mask, RA, RB, RT, RQ, store=False):
        ...
        for i, slice_i in enumerate([RA, RB, RT, RQ]):
            for j, slice_j in enumerate([RA, RB, RT, RQ]):
...
class GraphTransformer(nn.Module):
    def __init__(self, dim, n_layers, final_dim, d_inner, 
        ...
        self.seg_embedding = nn.Parameter(torch.rand(4,1,dim).normal_(0, 0.1))
        ...
        self.layers = nn.ModuleList([GraphLayer(d_model=dim, d_inner=d_inner, n_head=n_head, d_head=dim//n_head, 
                               dropout=dropout, attn_dropout=dropatt, wnorm=wnorm, lev=i+1) for i in range(n_layers)])
    ...
    def forward(self,x_atom,x_atom_pos, x_bond, x_bond_dist, x_triplet, x_triplet_angle, x_quad, x_quad_angle):
        ...
        new_mask[mask > 0] = 0
        Z = torch.cat([
            self.atom_embedding(x_atom[:,:,:3], x_atom_pos[:,:,3:]),     #  + self.seg_embedding[:1],
            self.bond_embedding(x_bond[:,:,:3], x_bond_dist),            #  + self.seg_embedding[1:2],
            self.triplet_embedding(x_triplet[:,:,:2], x_triplet_angle),  #  + self.seg_embedding[2:3],
            self.quad_embedding(x_quad[:,:,:1], x_quad_angle),           #  + self.seg_embedding[3:4]
            ], dim=1)
